...
@Component({
  selector: 'app-daily-routine-dialog',
  templateUrl: './daily-routine-dialog.component.html',
  styleUrls: ['./daily-routine-dialog.component.scss']
})
export class DailyRoutineDialogComponent implements OnInit{
...
filteredWards: string[] = [];
  ...
  constructor(
    private fb: FormBuilder,
        public dialogRef: MatDialogRef<DailyRoutineDialogComponent>,
...
private _filterWard(value: string): string[] {
  ...
  getadmitdata()  {
    ...
    this.firebaseCollectionService.getadmitList(userId, clinicId, ReceptionistId, 'admitlist').then((appointment) => {
      if (appointment && appointment.length > 0) {
        this.admitlist = appointment

         this.filteredAppointmentsList = [...this.admitlist];
        console.log("this.appointmentslist",this.admitlist);
      }
    })
  }
...
filterPatients(event: Event) {
  ...
  this.filteredAppointmentsList = this.admitlist.filter(patient => {
    ...
    doAction(){
       const payload = {
         ...
         reasonforAdmit: this.admitPatientForm.value.reasonforAdmit,
          heaithReports: this.admitPatientForm.value.heaithReports,
  ...
  const selectedPatient = this.admitlist.find(
    p => (p.patientName ) === event.value
  );
  ...
  if (selectedPatient) {
    this.admitPatientForm.patchValue({
      ...
      reasonforAdmit: selectedPatient.reasonforAdmit || '',
    });
  }
}

getheaithReports(): FormArray {
    return this.admitPatientForm.get('heaithReports') as FormArray;
  }
